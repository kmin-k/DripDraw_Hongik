...
PR을 열거나 `dev`·`main`에 push하면 위 명령이 **GitHub에서 자동으로 실행**됩니다
([`.github/workflows/ci.yml`](.github/workflows/ci.yml)). PR 화면 아래에 결과가 ✓ / ✗로 표시됩니다.

- 새로운 검사를 추가한 것이 아니라 **위 명령을 그대로 돌립니다.** 한쪽만 바꾸면 어긋나므로 함께 고치세요
- 포맷은 `--check`로 확인만 합니다. CI가 코드를 고치지 않으므로 **올리기 전에 로컬에서 포맷**하세요
- 프로덕션 빌드(`npm run build`)도 함께 돕니다. 개발 서버에서는 안 드러나는 타입·번들 오류를 잡습니다

✗가 뜨면 로그에서 실패한 단계를 열어 같은 명령을 로컬에서 재현할 수 있습니다.
