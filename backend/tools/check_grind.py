...
def main() -> int:
    ...
    ap.add_argument("--no-watershed", action="store_true",
                    help="입자 분리를 끄고 비교해 본다")
    ap.add_argument("--debug", default="debug_result.jpg",
    ...
    print()
    print("  개수 가중   D10 %7.1f   D50 %7.1f   D90 %7.1f µm"
          % (result.d10_um, result.d50_um, result.d90_um))
    print("  부피 가중  Dv10 %7.1f  Dv50 %7.1f  Dv90 %7.1f µm"
          % (result.dv10_um, result.dv50_um, result.dv90_um))
    print("  균일도(span) %.3f   |   추정 구간: %s" % (result.span, result.grind_label))
