...
def make(path="synthetic.jpg", um_per_px=8.4, marker_mm=10.0,
         mean_um=800.0, sigma=0.30, n=600, perspective=True, seed=7):
    ...
    while len(truth) < n and tries < n * 60:
        ...
        if any((cx - px) ** 2 + (cy - py) ** 2 < (r + pr) ** 2 * 0.55
        ...
        by = r * r / ax
        cv2.ellipse(img, (int(cx), int(cy)), (int(round(ax)), int(round(by))),
                    ang, 0, 360, (int(rng.integers(25, 60)),) * 3, -1, cv2.LINE_AA)
        truth.append(dia_um)

    # 비네팅(조명 불균일)
    yy, xx = np.mgrid[0:H, 0:W]
    v = 1.0 - 0.35 * (((xx - W * 0.4) / W) ** 2 + ((yy - H * 0.55) / H) ** 2) * 3.2
    img = np.clip(img.astype(np.float32) * np.clip(v, 0.55, 1.0)[..., None], 0, 255).astype(np.uint8)
    ...
    vw = t ** 3
    o = np.argsort(t)
    cw = np.cumsum(vw[o]); cw = cw / cw[-1]
    print(f"[정답] n={t.size}  D50={np.percentile(t,50):.0f}µm  "
