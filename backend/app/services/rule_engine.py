...
@dataclass(frozen=True)
class PourPlan:
    """주수 배분 결과. 규칙 생성과 피드백 보정이 함께 씁니다."""
...
def _grind_guide(drink_type: str, d50_um: float) -> str:
    ...
    low, high = C.D50_RANGE[drink_type]
    if d50_um < low:
        steps = round_half_up((low - d50_um) / C.GRIND_STEP_UM)
        return f"{steps}단계 굵게" if steps else "현재 분쇄도 유지"
    if d50_um > high:
        steps = round_half_up((d50_um - high) / C.GRIND_STEP_UM)
...
def build_pour_plan(
    *,
    ...
    """물량·유량·간격으로 주수 배분과 Target Curve를 만듭니다 (6절).

    규칙으로 생성한 레시피와 피드백으로 보정한 레시피가 **같은 함수로** 곡선을 그립니다.
    보정 쪽에 곡선 계산을 복제하면 두 곡선의 형태가 서서히 어긋납니다.
    ...
    longest_pour_sec = second / flow_gps
    if longest_pour_sec > interval_sec:
        raise RuleViolation(
            f"한 번에 붓는 시간({longest_pour_sec:.1f}초)이 "
            f"주수 간격({interval_sec}초)을 넘습니다. "
            f"현재 원두량 {dose_g} g에서는 물을 더 늘릴 수 없습니다."
        )

    # 주수는 간격마다 시작합니다. 대기는 간격에서 푸어 시간을 뺀 나머지입니다.
    starts = [float(interval_sec * i) for i in range(4)]
    amounts = [bloom_water_g, second, third, fourth]
    ...
    for phase, amount, start in zip(phases, amounts, starts, strict=True):
        ...
        duration = C.BLOOM_POUR_SEC if phase == "BLOOM" else amount / flow_gps
        pours.append(Pour(phase, amount, round_half_up(start), round_half_up(start + duration)))

    # 마지막 주수를 시작한 뒤 물이 다 빠질 때까지가 총 추출 시간입니다.
    total_time = round_half_up(starts[-1] + C.DRAWDOWN_SEC)
    ...
    return PourPlan(pours=pours, total_time_sec=total_time, target_curve=curve)
...
def generate_recipe(
    *,
    ...
    ratio_override로 기본 Ratio 대신 다른 값을 넣을 수 있습니다.
    Ratio가 오르면 한 번에 부을 물량이 늘어 푸어가 주수 간격을 넘길 수 있는데,
    그 검사는 build_pour_plan이 합니다 (8-4절).
    """
