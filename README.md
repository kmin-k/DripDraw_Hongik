...
> **개발 서버(5180)가 아니라 빌드본(4173)이어야** 설치 아이콘이 나타납니다.
> 백엔드는 이때도 8000번으로 함께 띄워야 합니다.

빌드본에서만 설치가 동작합니다(개발 서버에서는 서비스 워커를 등록하지 않습니다).
안드로이드 크롬은 주소창 메뉴의 **앱 설치**, 데스크톱 크롬은 주소창 오른쪽 설치 아이콘을 누르면
홈 화면·시작 메뉴에 추가되고 주소창 없이 전체화면으로 열립니다.
