...
배포를 한다면 얻는 것과 드는 값은 이렇습니다.

| 얻는 것 | 드는 값 |
|---|---|
| 심사위원이 직접 접속해 볼 수 있음 | HTTPS 필수 — **Web Bluetooth는 보안 컨텍스트에서만 동작**합니다 |
| "동작하는 서비스"라는 인상 | 저울은 접속자 손에 없으므로 원격 접속자는 추출 화면을 쓸 수 없음 |
| 팀원이 각자 확인 가능 | 서버 비용·계정·CI 관리 부담 |

**핵심 제약**: 배포해도 저울 연동은 접속한 사람의 브라우저 ↔ 그 사람의 저울 사이에서만 일어납니다. 배포가 시연을 대체하지 못합니다.

### DB 전환 (SQLite → MySQL 등)

현재 SQLite는 **파일 하나**라 서버를 따로 띄울 필요가 없고, 단일 사용자 데모 범위에서 부족한 점이 없습니다.

전환이 필요해지는 조건은 **동시 사용자**입니다. SQLite는 쓰기를 한 번에 하나만 처리하므로, 여러 명이 동시에 추출을 저장하는 상황이 생기면 그때 바꿉니다. 지금은 그런 상황이 없습니다.

전환 비용은 낮게 유지되어 있습니다 — SQLAlchemy를 쓰고 있어 **접속 문자열과 일부 컬럼 타입만** 바뀝니다. 다만 **곡선을 담은 JSON 컬럼**은 DB마다 취급이 달라 확인이 필요합니다.
